...
class ContentProcessor:
    ...
    def extract_text(self, pdf_fn:str) -> str:
        ...
        with io.open(pdf_fn, "rb") as fd:
            files = {
                "input": (pdf_fn, fd, "application/pdf")
            }
            try:
                ...
                if response.status_code == 200:
                    ...
                    print("Grobid API error %s" % str(response.status_code), file=sys.stderr)
                    return ""
    ...
    def filter_abbreviations(self, abbreviation:str, ignore_lower:bool=False) -> str:

        # Ignoring anomalies such as capitalized, digit-only, lower-only, containing underscore and mono-letter abbreviation
        if not ignore_lower:
            if len(abbreviation[1:-1]) > 2 and sum(1 for e in abbreviation[1:-1] if e.isupper()) < 2:
                return None
    ...
    def format_abbreviation(self, abbreviation:str) -> str:
        ...
        abbreviation = abbreviation[:-2]+abbreviation[-1] if re.search(r'\([A-Z0-9]+s\)$', abbreviation) != None else abbreviation
...
class BertClassifier(torch.nn.Module):
    ...
    def __init__(self, model:str, dropout:float, from_pretrained:bool=True) -> None:
        super(BertClassifier, self).__init__()
        self.bert = BertModel.from_pretrained(model, local_files_only=True, torch_dtype="auto", device_map="auto")
...
class AbbreviationExtractor:
    ...
    def __init__(self, acr_regex:str, model_name:str, state_dict:str) -> None:
        ...
        self._tokenizer = BertTokenizer.from_pretrained(model_name, local_files_only=True, device_map="auto")
        self._classifier = BertClassifier(model_name, dropout=0.)
        ...
        if cuda_available:
            ...
            self._classifier.load_state_dict(torch.load(state_dict, weights_only=True, map_location=self._device))
    ...
    def _classify_abbreviation(self, path:str, abbreviations:list) -> list:
        ...
        # Tokenizing inputs
        tok_inputs = self._tokenizer(inputs, padding="max_length", max_length=512, truncation=True, return_tensors="pt")
        tok_inputs.to(self._device)

        # Classifying abbreviation
        with torch.no_grad():
            outputs = self._classifier(tok_inputs['input_ids'], tok_inputs['attention_mask']).argmax(dim=1).cpu().detach()
    ...
    def extract_and_classify_abbreviations(self, data:dict) -> dict:
        ...
        for k in files.keys():
            ...
            abbreviations = [self._cp.filter_abbreviations(e, ignore_lower=True) for e in abbreviations]
            ...
            for a in abbreviations:
                for i in range(len(split_content)):
                    if split_content[i] == a:
                        extracted_abbreviations.append(" ".join(split_content[i-len(a)+2:i+1]))

            # Classifying abbreviations
            if len(abbreviations) > 0:
                result += self._classify_abbreviation(k, extracted_abbreviations)
            else:
                print("Could not find abbreviations in %s" % json.dumps(k), file=sys.stderr)

        # Counting, sorting and exporting the results
        result = pd.DataFrame(result).value_counts()
        result = pd.DataFrame([list(result.index[i]) + [result.values[i]] for i in range(len(result.index))], columns=["Source", "Abbreviation", "X", "Count"])
